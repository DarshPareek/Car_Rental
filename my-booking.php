...
<table>
  ...
  <tr>
    ...
    <td>
        <?php
        // Calculate the total hours between FromDate and ToDate
        $fromDate = new DateTime($result->FromDate);
        $toDate = new DateTime($result->ToDate);
        $interval = $fromDate->diff($toDate);
        $totalHours = ($interval->days * 24) + $interval->h + ($interval->i / 60); // Total hours with minutes as fractional hours
    ...
    <td>
        <?php
        // Price per hour
        $pph = $result->PricePerDay; // Assuming you have an hourly rate in the database
        echo htmlentities($pph);
        ?>
    </td>
</tr>
<tr>
    <th colspan="4" style="text-align:center;"> Grand Total</th>
    <th><?php echo htmlentities(round($totalHours * $pph));?></th>
</tr>
</table>
<hr />
              <?php }}  else { ?>
